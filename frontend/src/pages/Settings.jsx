  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 20, borderBottom: "1px solid var(--border)" }}>
        {TABS.map(t => (
          <button key={t} onClick={() => { setTab(t); setMsg({ type: "", text: "" }); }} style={{
            ...
            fontFamily: "var(--font-head)", fontSize: "0.85rem", fontWeight: 600,
...
function GeneralTab({ setMsg }) {
  ...
  async function saveAllowlist() {
    try {
      ...
      setMsg({ type: "success", text: "✓ Allowlist updated" });
    } catch (e) { setMsg({ type: "danger", text: e.message }); }
  }
    ...
    <div className="grid-2">
      <Panel title="Allowlist">
        <div className="form-group">
          ...
          <textarea className="form-control" rows={4} value={allowlist}
            ...
            <div style={{ display: "grid", gap: 8 }}>
              {Object.entries(info).map(([k, v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <span className="mono text-accent" style={{ fontSize: "0.68rem", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{String(v)}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {Object.entries(stats).map(([k, v]) => (
                <div key={k} style={{ padding: "8px 10px", background: "var(--surface2)", border: "1px solid var(--border)" }}>
                  <div className="mono text-dim" style={{ fontSize: "0.58rem", letterSpacing: "0.15em", textTransform: "uppercase" }}>{k.replace(/_/g, " ")}</div>
                  <div style={{ fontFamily: "var(--font-head)", fontSize: "1.4rem", fontWeight: 800, color: "var(--text-bright)", marginTop: 2 }}>{v}</div>
                </div>
              ))}
            </div>
          )}
...
function UsersTab({ setMsg }) {
  ...
  async function load() {
    try { setUsers(await api("/settings/users")); }
    catch (e) { setMsg({ type: "danger", text: e.message }); }
  }
  useEffect(() => { load(); }, []);
  ...
  async function createUser() {
    ...
    try {
      ...
      setMsg({ type: "success", text: `✓ User ${email} created` });
          ...
          <tbody>
            {users.length ? users.map(u => (
              <tr key={u.id}>
                ...
                <td>
                  <button className="btn btn-ghost btn-sm" style={{ color: "var(--critical)", borderColor: "rgba(255,71,87,0.3)" }}
...
function IntegrationsTab({ setMsg }) {
  ...
  async function save() {
    ...
    try {
      ...
      setMsg({ type: "success", text: "✓ Integrations saved" });
    } catch (e) { setMsg({ type: "danger", text: e.message }); }
    finally { setLoading(false); }
  }
  ...
  async function testIntegration(type) {
    ...
    try {
      ...
      setMsg({ type: "success", text: `✓ ${type} test sent successfully` });
    } catch (e) { setMsg({ type: "danger", text: `${type} test failed: ${e.message}` }); }
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={cardStyle}>
          <div style={headerStyle}>
            <span style={{ fontSize: "1.2rem" }}>💬</span> Slack Webhook
          </div>
          <div className="form-group">
            <label className="form-label">Webhook URL</label>
            <input className="form-control" value={integrations.slack_webhook}
              onChange={e => update("slack_webhook", e.target.value)}
              placeholder="https://hooks.slack.com/services/..." />
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-outline btn-sm" onClick={() => testIntegration("slack")}
        ...
        <div style={cardStyle}>
          <div style={headerStyle}>
            <span style={{ fontSize: "1.2rem" }}>📧</span> Email (SMTP)
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
            <div className="form-group">
              <label className="form-label">SMTP Host</label>
              <input className="form-control" value={integrations.email_smtp_host}
                onChange={e => update("email_smtp_host", e.target.value)} placeholder="smtp.gmail.com" />
            </div>
            <div className="form-group">
              <label className="form-label">Port</label>
              <input className="form-control" value={integrations.email_smtp_port}
                onChange={e => update("email_smtp_port", e.target.value)} placeholder="587" />
            </div>
          </div>
          <div className="form-group">
            <label className="form-label">From Address</label>
            <input className="form-control" value={integrations.email_from}
              onChange={e => update("email_from", e.target.value)} placeholder="alerts@company.com" />
          </div>
          <div className="form-group">
            <label className="form-label">Password / App Token</label>
            <input className="form-control" type="password" value={integrations.email_password}
              onChange={e => update("email_password", e.target.value)} placeholder="App password" />
          </div>
          <div className="form-group">
            <label className="form-label">Send To</label>
            <input className="form-control" value={integrations.email_to}
              onChange={e => update("email_to", e.target.value)} placeholder="team@company.com" />
          </div>
          <button className="btn btn-outline btn-sm" onClick={() => testIntegration("email")}
        ...
        <div style={cardStyle}>
          <div style={headerStyle}>
            <span style={{ fontSize: "1.2rem" }}>🔗</span> Generic Webhook
          </div>
          <div className="form-group">
            <label className="form-label">Webhook URL</label>
            <input className="form-control" value={integrations.webhook_url}
          ...
          <button className="btn btn-outline btn-sm" onClick={() => testIntegration("webhook")}
            disabled={testLoading === "webhook"}>
        ...
        <div style={cardStyle}>
          <div style={headerStyle}>
            <span style={{ fontSize: "1.2rem" }}>⚙</span> API Access
          </div>
          ...
          <div style={{
            padding: 10, background: "#050a0e",
            border: "1px solid var(--border)",
            fontFamily: "var(--font-mono)", fontSize: "0.65rem",
            color: "var(--accent)", lineHeight: 1.8,
          }}>
      ...
      <button className="btn btn-primary btn-lg" onClick={save} disabled={loading} style={{ gap: 8 }}>
