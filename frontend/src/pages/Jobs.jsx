...
function validateTarget(target, scanType) {
  ...
  if (scanType === "external") {
    ...
    const isDomain =
      /^([a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$/.test(target);
    if (!isUrl && !isDomain)
      return "Enter a domain (e.g. example.com) or URL (https://example.com)";
    if (isUrl) {
      try { new URL(target); } catch { return "Invalid URL format"; }
    }
  } else {
    ...
    const isHostname =
      /^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z0-9-]+)*$/.test(target);
    if (!isIp && !isHostname) return "Enter a valid IP address or hostname";
  }
  return null;
}
...
export default function Jobs() {
  ...
  async function submit() {
    ...
    if (err) { setTargetErr(err); return; }
    setLoading(true);
    setMsg({ type: "", text: "" });
    try {
      const r = await api("/scan/jobs", {
        method: "POST",
        body: { target: target.trim(), profile_id: Number(profileId), scan_type: scanType },
      });
      setMsg({ type: "success", text: `✓ Scan job #${r.id} launched — ${target} [${scanType}]` });
      await load();
    } catch (e) {
  ...
  async function deleteJob(jobId) {
    ...
    try {
      ...
      setDeletingIds((s) => { const n = new Set(s); n.delete(jobId); return n; });
      await load();
  ...
  function toggleErrorExpand(jobId) {
    setExpandedErrors(s => {
      const n = new Set(s);
    ...
    <div className="fade-in">
      <div className="page-header">
        <div>
          ...
          <div className="page-desc">// Execute internal LAN scans and external web scans</div>
        </div>
      </div>
      ...
      <div className="grid-2" style={{ marginBottom: 20 }}>
        <Panel title="Launch Scan">
              ...
              {["internal", "external"].map((type) => (
                <button key={type} className="btn btn-ghost"
                  style={{
                    ...
                    borderColor: scanType === type ? "var(--accent)" : "var(--border)",
                    color: scanType === type ? "var(--accent)" : "var(--text-dim)",
                    background: scanType === type ? "rgba(0,212,255,0.08)" : "var(--surface2)",
                  }}
          ...
          {scanType === "external" && (
            <div style={{
              ...
              fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--medium)", lineHeight: 1.6,
            }}>
          ...
          <div className="form-group">
            <label className="form-label">
              {scanType === "external" ? "Domain or URL" : "IP Address or Hostname"}
            </label>
            <input className="form-control" value={target}
              onChange={(e) => handleTargetChange(e.target.value)}
              placeholder={scanType === "external" ? "https://example.com or example.com" : "10.0.0.1 or host.internal.local"}
              onKeyDown={(e) => e.key === "Enter" && submit()}
              style={targetErr ? { borderColor: "var(--critical)" } : undefined}
            />
            {targetErr && (
              <div style={{ color: "var(--critical)", fontFamily: "var(--font-mono)", fontSize: "0.65rem", marginTop: 4 }}>
                {targetErr}
              </div>
            )}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            <button className="btn btn-primary btn-lg" style={{ flex: 1, gap: 8 }}
              onClick={submit} disabled={loading || !profileId}
            >
              {loading && <div className="spinner" />} ⚡ LAUNCH SCAN
            </button>
        ...
        <Panel title="Scan Info">
          <div style={{ display: "grid", gap: 12 }}>
            <div>
              <div style={{ fontWeight: 600, color: "var(--text-bright)", marginBottom: 4 }}>🏠 Internal / LAN</div>
              <div style={{ fontSize: "0.83rem", color: "var(--text-dim)", lineHeight: 1.6 }}>
                Port scan, banner grab, SSH inventory, NVD CVE match, TLS check
              </div>
            </div>
            <div>
              <div style={{ fontWeight: 600, color: "var(--text-bright)", marginBottom: 4 }}>🌐 External / Web</div>
              <div style={{ fontSize: "0.83rem", color: "var(--text-dim)", lineHeight: 1.6 }}>
                HTTP fingerprint, web tech detect, favicon hash, CMS CVE map, CISA KEV correlation
              </div>
            </div>
          </div>
          <div style={{
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "var(--text-dim)", letterSpacing: "0.14em", marginBottom: 7 }}>
              REMEDIATION SLA
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5, fontFamily: "var(--font-mono)", fontSize: "0.72rem" }}>
      ...
      <Panel title="Job History" extra={
        ...
        <table className="data-table">
          <thead>
            <tr><th>ID</th><th>Target</th><th>Type</th><th>Profile</th><th>Status</th><th>Started</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {jobs.length ? jobs.map((j) => {
              const prof = profiles.find((p) => p.id === j.profile_id);
              const hasError = j.status === "failed" && j.error_info;
              const isExpanded = expandedErrors.has(j.id);
              return (
                <React.Fragment key={j.id}>
                  <tr>
                    ...
                    <td>
                      <span className={`badge ${j.scan_type === "external" ? "badge-success" : "badge-info"}`}>
                        {j.scan_type === "external" ? "🌐" : "🏠"} {j.scan_type || "internal"}
                      </span>
                    </td>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        {hasError && (
                          <button
                            ...
                            style={{ padding: "2px 6px", fontSize: "0.6rem", color: "var(--critical)", borderColor: "rgba(255,71,87,0.3)" }}
                            onClick={() => toggleErrorExpand(j.id)}
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                        ...
                        <button className="btn btn-ghost btn-sm"
                          ...
                          onClick={() => deleteJob(j.id)} disabled={deletingIds.has(j.id)} title="Delete job"
                        >🗑</button>
                      </div>
                    </td>
                  </tr>
                      ...
                      <td colSpan="7" style={{ padding: 0 }}>
                        <div style={{
                          ...
                          <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                            <span style={{ color: "var(--critical)", fontSize: "0.85rem", flexShrink: 0 }}>⚠</span>
                            <div style={{ flex: 1 }}>
                              {j.error_info.error_detail && (
                              ...
                              <div style={{
                                ...
                                color: "var(--critical)", background: "#050a0e",
